...
impl<Elem: DynArrElem> DynArr<Elem> {
    ...
    pub fn push(&mut self, elem: Elem) -> Result<(), DynArrError> {
        if self.count == DYN_ARR_CAPACITY { return Err(DynArrError::ReachedCapacity); }
    ...
    pub fn iter(&self) -> DynArrIter<'_, Elem> {
        DynArrIter {
            position: 0,
            array: self
        }
...
impl<Elem: DynArrElem> AddAssign for DynArr<Elem> {
    fn add_assign(&mut self, rhs: Self) {
        for elem in rhs.iter() {
            if self.push(elem) == Err(DynArrError::ReachedCapacity) { break; }
        }
    }
}
