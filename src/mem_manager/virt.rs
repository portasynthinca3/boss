...
struct RawTable<'s> {
    ...
    entries: *mut [TableEntry; ENTRY_CNT],
...
impl<'s> RawTable<'s> {
    ...
    unsafe fn get_downstream_by_idx(&self, idx: usize) -> Option<RawTable<'s>> {
        ...
        Some(Self {
            ...
            entries: virt_addr.0 as *mut [TableEntry; ENTRY_CNT],
            kind: ds_kind,
    ...
    unsafe fn set_downstream_by_idx(&mut self, idx: usize, table: Option<&mut RawTable>) -> Result<(), ()> {
        ...
        let Some(table) = table else {
            (*self.entries)[idx] = TableEntry::new().with_present(false);
            return Ok(());
        };
        ...
        // set entry
        (*self.entries)[idx] = table.to_parent_entry();
        Ok(())
    }
    ...
    /// also that we want to map the virtual address range `0b0101 ..= 0b1010`
...
impl Debug for RawTable<'_> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(f, "{:?} at {:?} ({:?} through {:?})", self.kind, self.addr(),
...
impl AddressSpace {
    ...
    pub fn new() -> Result<AddressSpace, MemMgrError> {
        ...
        Ok(AddressSpace {
            cr3: CR3::new().with_addr(table.addr().0 >> BITS_FOR_PAGE_OFFSET),
        })
    }
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        ...
        self.iterate_over_range(VirtAddr::from_usize(0)..=VirtAddr::from_usize(usize::MAX), |table, kind, addr| {
            ...
            if start_addr.is_none() {
                start_addr = Some((addr, table.as_ref().unwrap().addr()));
            }
        ...
        fn iterate_table(
            ...
            if table.kind.downstream().is_some() { // ensure that we can have children
                for i in table.downstream_idx_range(addresses.clone()).unwrap() {
                    ...
                    let original_address = downstream.as_ref().map(|t| t.addr());
                    ...
                    let address_changed = changes_applied
                                       ...
                                       && downstream.as_ref().map(|t| t.addr()) != original_address;
                    if table_removed || address_changed {
                        let mut pages = DynArr::<PhysAddr>::new();
        ...
        self.iterate_over_range_mut(start_virt..=end_virt, |child, kind, low_addr| {
            ...
            if kind == TableKind::Page {
                ...
                *child = Some(RawTable {
                    ...
                    entries: (start_phys.0 + (page_ctr * PAGE_SIZE)) as *mut [TableEntry; ENTRY_CNT],
                    attrs,
                    subspace: None,
                });
                page_ctr += 1;
